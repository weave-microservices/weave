...
const makeCallUtils = ({ state, log, options, registry, shouldCollectMetrics }) => {
    let transport = null
    ...
    function localCall (context, endpoint, opts) {
        ...
        if (context.metrics || state.statistics) {
    ...
    function finishCall (context) {
        ...
        if (state.statistics) {
            state.statistics.addRequest(context.action.name, context.duration)
        }
