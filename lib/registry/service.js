...
const makeServiceFactory = ({
    ...
    (schema) => {
        ...
        if (!schema) {
            throw new WeaveError('Must to set a schema for this service!')
        }
        ...
        if (!schema.name) {
            throw new WeaveError('Must to set a service name!')
        }
        ...
        if (schema.beforeCreate) {
            ...
            if (Array.isArray(schema.beforeCreate)) {
                Promise.all(schema.beforeCreate.map(beforeCreate => beforeCreate()))
            }
        }
        ...
        if (isObject(schema.events)) {
            forIn(schema.events, (event, name) => {
                ...
                if (!event.handler) {
                    throw new WeaveError('Missing event handler') // todo: better message
                }
                ...
                event.handler = (payload, sender) => {
                    return handler.apply(self, [payload, sender, event.name])
                }
        ...
        if (schema.created) {
            ...
            if (Array.isArray(schema.created)) {
                Promise.all(schema.created.map(createdHook => createdHook()))
            }
        }
