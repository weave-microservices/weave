...
const MakeRegistry = ({ state, transport, getLogger, bus, wrapAction }) => {
    const self = Object.create({})

    self.log = getLogger('REGISTRY')
    self.nodes = NodeCatalog({ registry: self, log: self.log, transport, state, bus })
    ...
    // self.findNextActionEndpoint = (action, opts = {}) => {
    //     if (typeof action !== 'string') {
    //         return action
    //     } else {
    //         if (opts.nodeId) {
    //             const endpoint = self.getActionEndpointByNodeId(action, opts.nodeId)
    //             if (!endpoint) {
    //             }
    //         } else {
