...
export function createWatchMiddleware(weaveCli: WeaveCli, options: WatcherOptions = {}) {
  return function watchMiddleware(runtime: WatchRuntime) {
        ...
        if (parents && parents.length > 0) {
          ...
          if (serviceFile) {
            ...
            if (service && !watchItem.services.includes(service.fullyQualifiedName)) {
              ...
              runtime.log.debug(
                `Dependency "${relativePath}" linked to service "${service.fullyQualifiedName}"`,
              );
            }
          }
        }
    ...
    function watchProjectFiles(): void {
      ...
      const configFiles = ["weave.config.js", "weave.config.ts", "weave.config.json"];
      ...
      const reloadServices = debounce(() => {
        ...
        runtime.log.info(
          `Reload ${needToReload.size} ${needToReload.size > 1 ? "services" : "service"}`,
        );
      ...
      projectFiles.forEach((watchItem, filename) => {
        try {
          ...
          if (watchItem.restartBroker) {
            ...
            runtime.log.debug(
              `Reloading ${watchItem.services.length} services... {${relativePath}}`,
            );
          }
