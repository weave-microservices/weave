...
const groupedMiddlewareFunctions = [
  ...
  },
  {
    ...
    hooks: ["ping", "call", "multiCall", "emit", "broadcast", "broadcastLocal"],
  },
  {
    groupName: "Service hooks",
    hooks: ["serviceCreating", "serviceCreated", "serviceStopping", "serviceStopped"],
  },
  {
    groupName: "Service action/event hooks",
    hooks: ["localAction", "localEvent", "remoteAction", "remoteEvent"],
  },
  {
    groupName: "Transport hooks",
    hooks: ["transportSend", "transportMessageHandler"],
  },
];

export default async (middlewareName: string, options: CreateCommandOptions): Promise<void> => {
    ...
    const result = await inquirer.prompt([
      {
        ...
        choices: groupedMiddlewareFunctions
          .map((group) => {
  ...
  ejs.renderFile(
    path.join(__dirname, "templates", "middleware.ejs"),
    data,
    null,
