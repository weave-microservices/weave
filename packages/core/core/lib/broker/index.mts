...
export const createBrokerInstance = (runtime: Runtime): Broker => {
  ...
  broker.getNextActionEndpoint = function (actionName: string, options: Record<string, unknown> = {}): Endpoint | Error {
  ...
  broker.broadcastLocal = eventBus!.broadcastLocal.bind(broker) as (eventName: string, payload?: unknown, options?: EventOptions) => Promise<void>;
  ...
  broker.loadServices = function (folder: string = './services', fileMask: string = '*.service.js'): number {
  ...
  */
  broker.start = async function (): Promise<void> {
    ...
    const failedServices = serviceStartResults
      ...
      .filter(({ result }) => result.status === 'rejected') as Array<{ result: PromiseRejectedResult; service: Service }>;
    ...
    if (failedServices.length > 0) {
      ...
      if (failedServices.some(({ service }) => (service.schema as any).critical !== false)) {
        const criticalFailures = failedServices.filter(({ service }) => (service.schema as any).critical !== false);
  ...
  */
  broker.ping = function (nodeId?: string, timeout: number = 3000): Promise<PingResult | Record<string, PingResult | null> | null> {
    if (transport && transport.isConnected) {
      if (nodeId) {
        ...
        const nodes = (registry as Registry & { nodeCollection: NodeCollection }).nodeCollection.list({})
  ...
  const registerMiddlewares = (customMiddlewares?: Middleware[]): void => {
    ...
    (runtime.eventBus as EventBus & { broadcastLocal: (eventName: string, payload?: unknown, options?: EventOptions) => void }).broadcastLocal = middlewareHandler!.wrapMethod('broadcastLocal', runtime.eventBus!.broadcastLocal);
  ...
  if (isFunction(options.beforeRegisterMiddlewares)) {
    (options.beforeRegisterMiddlewares as (this: Broker, args: { broker: Broker; runtime: Runtime }) => void).call(broker, { broker, runtime });
  }
