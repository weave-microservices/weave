  ...
  const cache = {
    ...
    getCachingKey (actionName, data, metadata, keys) {
      if (data || metadata) {
        ...
        if (keys) {
          if (keys.length === 1) {
            const cacheKeyData = getPropertyFromDataOrMetadata.getPropertyFromDataOrMetadata(data, metadata, keys[0]);
          ...
          if (keys.length > 0) {
            const valueString = keys.reduce((pre, property, index) => {
              let value = getPropertyFromDataOrMetadata.getPropertyFromDataOrMetadata(data, metadata, property);
