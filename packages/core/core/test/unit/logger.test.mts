...
describe('Test logger module.', () => {
  ...
  it('Simple console transport', () => {
    ...
    if (tty.isatty(0)) {
      ...
      assert.deepStrictEqual(calls[0], ['{"level":40,"time":0,"service":"test","version":1,"message":"test"}' + os.EOL]);
    }
  ...
  it('Should log types', () => {
    ...
    if (tty.isatty(0)) {
      ...
      assert.deepStrictEqual(strippedMessage[0], 'INFO [1970-01-01T00:00:00.000Z] \n{\n  "0": "item1",\n  "1": "item2"\n}' + os.EOL);
      assert.deepStrictEqual(strippedMessage[1], 'INFO [1970-01-01T00:00:00.000Z] \n{\n  "user": "hans",\n  "rooms": [\n    1,\n    2,\n    3,\n    4\n  ],\n  "lastLogin": "2021-03-31T13:41:01.210Z",\n  "settings": {\n    "app": {\n      "darkMode": true,\n      "lang": "de"\n    }\n  }\n}' + os.EOL);
    } else {
      assert.deepStrictEqual(calls[0], ['{"0":"item1","1":"item2","level":40,"time":0,"service":"test","version":1}' + os.EOL]);
      assert.deepStrictEqual(calls[1], ['{"level":40,"time":0,"service":"test","version":1,"user":"hans","rooms":[1,2,3,4],"lastLogin":"2021-03-31T13:41:01.210Z","settings":{"app":{"darkMode":true,"lang":"de"}}}' + os.EOL]);
    }
  ...
  it('Should log multiple messages', () => {
    ...
    if (tty.isatty(0)) {
      const strippedMessage = stripMessages(calls);
      assert.deepStrictEqual(strippedMessage[0], 'INFO [1970-01-01T00:00:00.000Z]  message1 message2' + os.EOL);
    } else {
      assert.deepStrictEqual(calls[0], ['{"level":40,"time":0,"service":"test","version":1,"message":"message1 message2"}' + os.EOL]);
    }
    process.stdout.write = originalWrite;
  });
  ...
  it('Should log fatal messages', () => {
    ...
    if (tty.isatty(0)) {
      const strippedMessage = stripMessages(calls);
      assert.deepStrictEqual(strippedMessage[0], 'FATAL [1970-01-01T00:00:00.000Z]  Fatal error' + os.EOL);
    } else {
      assert.deepStrictEqual(calls[0], ['{"level":10,"time":0,"service":"test","version":1,"message":"Fatal error"}' + os.EOL]);
    }
    process.stdout.write = originalWrite;
  });
  ...
  it('Should log fatal errors', () => {
    ...
    if (tty.isatty(0)) {
      const strippedMessage = stripMessages(calls);
      assert.deepStrictEqual(strippedMessage[0], 'FATAL [1970-01-01T00:00:00.000Z]  override message\n{\n  "stack": "Here could be your stack!",\n  "type": "Error"\n}' + os.EOL);
    } else {
  ...
  it('Should handle custom log levels', () => {
    ...
    if (tty.isatty(0)) {
      const strippedMessage = stripMessages(calls);
      assert.deepStrictEqual(strippedMessage[0], 'BORING [1970-01-01T00:00:00.000Z]  This is a really boring message' + os.EOL);
    } else {
      const logObj = JSON.parse(calls[0]);
      assert.strictEqual(calls.length, 1);
...
describe('Log formatter', () => {
  it('should format log messages', () => {
    ...
    // Object value
    const objString = formatUtils.format('Log: %o/%o %O', ['Donald', { settings: { enabled: true }}], undefined);
