...
export function isStreamObjectMode(obj: unknown): boolean {
  ...
  if (stream.readableObjectMode || stream._readableState) {
    ...
    return !!(stream.readableObjectMode === true || (readableState && readableState.objectMode === true));
    ...
    return !!(stream.writableObjectMode === true || (writableState && writableState.objectMode === true));
  } else {
    return false;
  }
}
