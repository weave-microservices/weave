 ...
 * @param sampleTime - Sample time in milliseconds between measurements
 * @returns Promise resolving to CPU usage statistics
 * @example
 * const usage = await cpuUsage(200);
 * console.log(`Average: ${usage.avg}%`);
 * console.log(`Per core: ${usage.usages}%`);
 */
export function cpuUsage(sampleTime: number = 100): Promise<CPUUsage> {
