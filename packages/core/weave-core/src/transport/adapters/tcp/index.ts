...
export function TCPGossip(adapterOptions): TCPTransportAdapter {
  ...
  self.connect = async () => {
    ...
    (self.broker.registry.nodeCollection.localNode as TCPNode).port = port
  ...
  self.sendHello = (nodeId) => {
    ...
    const localNode = self.broker.registry.nodeCollection.localNode as TCPNode
  ...
  function addDiscoveredNode (nodeId, host, port) {
    const node: TCPNode = self.broker.registry.nodeCollection.createNode(nodeId)
  ...
  function startDiscoveryServer (port) {
    swim.bus.on('message', ({ nodeId, host, port }) => {
      if (nodeId && nodeId !== self.broker.nodeId) {
        let node: TCPNode = self.broker.registry.nodeCollection.get(nodeId)
        if (!node) {
