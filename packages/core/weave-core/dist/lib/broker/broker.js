...
function createBroker(options) {
    ...
    const createLogger = (moduleName, service) => {
        ...
        if (process.env.NODE_ENV === 'production') {
            options.logger.logLevel = options.logger.logLevel || logger_1.LogLevel.Info;
        }
        else if (process.env.NODE_ENV === 'test') {
            options.logger.logLevel = options.logger.logLevel || logger_1.LogLevel.Error;
        }
        else {
            options.logger.logLevel = options.logger.logLevel || logger_1.LogLevel.Debug;
        }
    ...
    // Log Messages
    log.info(`Initializing #weave node version ${package_json_1.version}`);
    ...
    // Metrics module
    broker.metrics = metrics_1.MetricsStorage(broker, options.metrics);
    broker.metrics.init();
    ...
    if (options.cache.enabled) {
        const createCache = cache_1.default.resolve(options.cache.adapter);
    ...
    if (options.tracing.enabled) {
        broker.tracer = tracing_1.Tracer();
