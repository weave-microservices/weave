...
exports.createServiceFromSchema = void 0;
...
*/
function createAction(broker, service, actionDefinition, name) {
    let action = actionDefinition;
    // if the handler is a method (short form), we wrap the method in our handler object.
    if (utils_1.isFunction(actionDefinition)) {
        action = utils_1.wrapHandler(actionDefinition);
    }
    else if (utils_1.isObject(actionDefinition)) {
        action = utils_1.clone(actionDefinition);
    }
    else {
        broker.handleError(new errors_1.WeaveError(`Invalid action definition in "${name}" on service "${service.name}".`));
    }
    const handler = action.handler;
    // Action handler has to be a function
    if (!utils_1.isFunction(handler)) {
        broker.handleError(new errors_1.WeaveError(`Missing action handler in "${name}" on service "${service.name}".`));
    }
    ...
    action.handler = utils_1.promisify(handler.bind(service));
    return action;
}
;
function createEvent(broker, service, eventDefinition, name) {
    let event;
    // if the handler is a method (short form), we wrap the method in our handler object.
    if (utils_1.isFunction(eventDefinition)) {
        event = utils_1.wrapHandler(eventDefinition);
    }
    else if (utils_1.isObject(eventDefinition)) {
        event = utils_1.clone(eventDefinition);
    }
    else {
        broker.handleError(new errors_1.WeaveError(`Invalid event definition "${name}" on service "${service.name}".`));
    }
    // Event handler has to be a function
    if (!utils_1.isFunction(event.handler) && !Array.isArray(event.handler)) {
        broker.handleError(new errors_1.WeaveError(`Missing event handler for "${name}" on service "${service.name}".`));
    }
    event.service = service;
    let handler;
    if (utils_1.isFunction(event.handler)) {
        handler = utils_1.promisify(event.handler.bind(service));
    }
    else if (Array.isArray(event.handler)) {
        handler = event.handler.map(h => {
            return utils_1.promisify(h.bind(service));
        });
    }
    if (!event.name) {
        event.name = name;
...
;
function applyMixins(service, schema) {
    const mixins = utils_1.wrapInArray(schema.mixins);
    if (mixins.length > 0) {
        const mixedSchema = Array
            ...
            return s ? options_1.mergeSchemas(s, mixin) : mixin;
        }, null);
        return options_1.mergeSchemas(mixedSchema, schema);
    }
    return schema;
}
;
 ...
 */
function createServiceFromSchema(broker, middlewareHandler, addLocalService, registerLocalService, schema) {
    ...
    if (!schema) {
        broker.handleError(new errors_1.WeaveError('Schema is missing!'));
    }
    ...
    // validate name
    if (!schema.name) {
        broker.handleError(new errors_1.WeaveError('Service name is missing!'));
    }
    // Set service version
    ...
    if (schema.beforeCreate) {
        if (utils_1.isFunction(schema.beforeCreate)) {
    ...
    if (utils_1.isObject(schema.methods)) {
        Object.keys(schema.methods).map(name => {
            ...
            if (['log', 'actions', 'meta', 'events', 'settings', 'methods', 'dependencies', 'version', 'dependencies', 'broker', 'created', 'started', 'stopped'].includes(name)) {
                broker.handleError(new errors_1.WeaveError(`Invalid method name ${name} in service ${service.name}.`));
            }
            service[name] = method.bind(service);
        });
    }
    // Bind and register service actions
    if (utils_1.isObject(schema.actions)) {
        Object.keys(schema.actions).map(name => {
            ...
            const innerAction = createAction(broker, service, utils_1.clone(actionDefinition), name);
    ...
    if (utils_1.isObject(schema.events)) {
    ...
    if (schema.created) {
        if (utils_1.isFunction(schema.created)) {
    ...
    service.start = () => {
        return Promise.resolve()
            ...
            if (utils_1.isFunction(schema.started)) {
                return utils_1.promisify(schema.started.bind(service))();
            }
            if (Array.isArray(schema.started)) {
                return schema.started
                    .map(hook => utils_1.promisify(hook.bind(service)))
    ...
    service.stop = () => {
        ...
        return Promise.resolve()
            ...
            .then(() => {
            if (utils_1.isFunction(schema.stopped)) {
                return utils_1.promisify(schema.stopped.bind(service))();
            }
...
exports.createServiceFromSchema = createServiceFromSchema;
;
