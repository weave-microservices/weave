...
const utils_1 = __importDefault(require("@weave-js/utils"));
 ...
 */
function createTransport(broker, adapter, middlewareHandler) {
    ...
    const doRequest = (context, resolve, reject) => {
        const isStream = utils_1.default.isStream(context.data);
        const request = {
        ...
        if (isStream && utils_1.default.isStreamObjectMode(context.data)) {
            payload.meta = payload.meta || {};
            payload.meta.$isObjectModeStream = true;
        }
        const message = transport.createMessage(message_types_1.default.MESSAGE_REQUEST, context.nodeId, payload);
        return transport.send(message)
            .then(() => {
            if (isStream) {
                const stream = context.data;
                payload.meta = {};
                if (utils_1.default.isStreamObjectMode(context.data)) {
                    payload.meta.$isObjectModeStream = true;
                }
                stream.on('data', data => {
                    stream.pause();
                    ...
                    for (const chunk of chunks) {
                        ...
                        const message = transport.createMessage(message_types_1.default.MESSAGE_REQUEST, context.nodeId, payloadCopy);
    ...
    transport.disconnect = () => {
        ...
        transport.isReady = false;
        stopTimers();
        const message = transport.createMessage(message_types_1.default.MESSAGE_DISCONNECT);
        return transport.send(message)
            .then(() => adapter.close());
    };
    transport.setReady = () => {
    ...
    */
    transport.sendNodeInfo = (sender) => {
        ...
        const message = transport.createMessage(message_types_1.default.MESSAGE_INFO, sender, info);
        return transport.send(message);
    };
    /**
    * Send a message
    ...
    */
    ...
    transport.sendPing = (nodeId) => {
        const pingMessage = transport.createMessage(message_types_1.default.MESSAGE_PING, nodeId, { dispatchTime: Date.now() });
        return transport.send(pingMessage);
    };
    transport.discoverNodes = () => {
        const discoveryMessage = transport.createMessage(message_types_1.default.MESSAGE_DISCOVERY);
        return transport.send(discoveryMessage);
    };
    transport.discoverNode = (target) => {
        const discoveryMessage = transport.createMessage(message_types_1.default.MESSAGE_DISCOVERY, target);
        return transport.send(discoveryMessage);
    };
    transport.sendEvent = (context) => {
        ...
        const message = transport.createMessage(message_types_1.default.MESSAGE_EVENT, context.endpoint ? context.nodeId : null, payload);
        return transport.send(message);
    };
    transport.sendBroadcastEvent = (nodeId, eventName, data, groups) => {
        ...
        const message = transport.createMessage(message_types_1.default.MESSAGE_EVENT, nodeId, payload);
        return transport.send(message);
    };
    ...
    transport.removePendingRequestsByNodeId = (nodeId) => {
        ...
        pending.requests.forEach((request, requestId) => {
            ...
            request.reject(new errors_1.WeaveError(`Remove pending requests for node ${nodeId}.`));
            pending.requestStreams.delete(requestId);
            pending.responseStreams.delete(requestId);
        });
    };
    transport.createMessage = (type, targetNodeId, payload) => {
        return {
            type: type || message_types_1.default.MESSAGE_UNKNOWN,
            targetNodeId,
            payload: payload || {}
        };
    };
    transport.request = (context) => {
        ...
        if (broker.options.transport.maxQueueSize && broker.options.transport.maxQueueSize < pending.requests.size) {
            return Promise.reject(new errors_1.WeaveQueueSizeExceededError({
                action: context.action.name,
    ...
    transport.response = (target, contextId, data, meta, error) => {
        ...
        const isStream = utils_1.default.isStream(data);
        const payload = {
            id: contextId,
            meta,
            data,
        ...
        if (isStream) {
            ...
            if (utils_1.default.isStreamObjectMode(data)) {
                payload.meta = payload.meta || {};
                payload.meta.$isObjectModeStream = true;
            }
            stream.pause();
            ...
            stream.on('data', data => {
                ...
                for (const chunk of chunks) {
                    ...
                    const message = transport.createMessage(message_types_1.default.MESSAGE_RESPONSE, target, payloadCopy);
                    transport.send(message);
                }
                // resume stream
                stream.resume();
            ...
            payload.data = null;
            const message = transport.createMessage(message_types_1.default.MESSAGE_RESPONSE, target, payload);
            return transport.send(message)
                .then(() => stream.resume());
        }
        const message = transport.createMessage(message_types_1.default.MESSAGE_RESPONSE, target, payload);
    ...
    const onConnect = ({ wasReconnect = false, useHeartbeatTimer = true, useRemoteNodeCheckTimer = true, useOfflineCheckTimer = true }) => Promise.resolve()
        ...
        .then(() => utils_1.default.promiseDelay(Promise.resolve(), 500))
        .then(() => {
    ...
    const messageHandler = message_handlers_1.createMessageHandler(broker, transport, pending);
    ...
    function makeSubscriptions() {
        return Promise.all([
            adapter.subscribe(message_types_1.default.MESSAGE_DISCOVERY),
            adapter.subscribe(message_types_1.default.MESSAGE_DISCOVERY, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_INFO),
            adapter.subscribe(message_types_1.default.MESSAGE_INFO, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_REQUEST, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_RESPONSE, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_PING, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_PONG, nodeId),
            adapter.subscribe(message_types_1.default.MESSAGE_DISCONNECT),
            adapter.subscribe(message_types_1.default.MESSAGE_HEARTBEAT),
            adapter.subscribe(message_types_1.default.MESSAGE_EVENT, nodeId)
        ]);
    }
    ...
    function sendHeartbeat() {
        ...
        const message = transport.createMessage(message_types_1.default.MESSAGE_HEARTBEAT, null, payload);
        transport.send(message);
    }
