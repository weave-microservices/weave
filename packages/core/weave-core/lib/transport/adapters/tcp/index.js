...
const index_1 = require("./discovery/index");
const message_types_1 = __importDefault(require("../../message-types"));
const tcpReader_1 = require("./tcpReader");
const tcpWriter_1 = require("./tcpWriter");
...
function TCPGossip(adapterOptions) {
    adapterOptions = utils_1.defaultsDeep(adapterOptions, defaultOptions);
    const self = adapter_base_1.default();
    let tcpReader;
    let tcpWriter;
    let gossipTimer;
    let swim;
    self.afterInit = function () {
        swim = index_1.createDiscoveryService(self, adapterOptions);
    };
    self.connect = () => __awaiter(this, void 0, void 0, function* () {
        const port = yield startTCPServer();
        yield startDiscoveryServer(port);
        yield startTimers();
    ...
    self.send = (message) => {
        if (!message.targetNodeId || ![
            message_types_1.default.MESSAGE_PING,
            message_types_1.default.MESSAGE_PONG,
            message_types_1.default.MESSAGE_EVENT,
            message_types_1.default.MESSAGE_REQUEST,
            message_types_1.default.MESSAGE_RESPONSE,
            message_types_1.default.MESSAGE_GOSSIP_HELLO,
            message_types_1.default.MESSAGE_GOSSIP_REQUEST,
            message_types_1.default.MESSAGE_GOSSIP_RESPONSE
        ].includes(message.type)) {
            if (message.type === message_types_1.default.MESSAGE_DISCONNECT) {
    ...
    self.sendHello = (nodeId) => {
        ...
        const message = self.transport.createMessage(message_types_1.default.MESSAGE_GOSSIP_HELLO, nodeId, {
    ...
    const onIncomingMessage = (type, data, socket) => {
        switch (type) {
            case message_types_1.default.MESSAGE_GOSSIP_HELLO:
                return onGossipHelloMessage(data, socket);
            case message_types_1.default.MESSAGE_GOSSIP_REQUEST:
                return onGossipRequestMessage(data, socket);
            case message_types_1.default.MESSAGE_GOSSIP_RESPONSE:
    ...
    function startTCPServer() {
        tcpReader = tcpReader_1.createTCPReader(self, adapterOptions);
        tcpWriter = tcpWriter_1.createTCPWriter(self, adapterOptions);
    ...
    function sendGossipRequestToRandomEndpoint(payload, nodes) {
        ...
        if (destinationNode) {
            const message = self.transport.createMessage(message_types_1.default.MESSAGE_GOSSIP_REQUEST, destinationNode.id, payload);
    ...
    function onGossipRequestMessage(data, socket) {
        try {
            ...
            if (response.online || response.offline) {
                ...
                const message = self.transport.createMessage(message_types_1.default.MESSAGE_GOSSIP_RESPONSE, destinationNode.id, response);
                self.send(message).catch(() => { });
            }
        }
        catch (error) {
