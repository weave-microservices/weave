...
const context_1 = require("../broker/context");
const message_types_1 = __importDefault(require("./message-types"));
function createMessageHandler(broker, transport, pending) {
    ...
    const onRequest = (payload) => {
        ...
        try {
            ...
            const context = context_1.createContext(broker);
    ...
    const onResponse = payload => {
        ...
        pending.requests.delete(payload.id);
        if (!payload.success) {
            let error = errors_1.restoreError(payload.error);
            if (!error) {
    ...
    const onPing = payload => {
        const message = transport.createMessage(message_types_1.default.MESSAGE_PONG, payload.sender, {
    ...
    return (type, data) => {
        try {
            if (data === null) {
                broker.handleError(new errors_1.WeaveError('Packet missing!'));
            }
            const message = data;
            const payload = message.payload;
            if (!payload) {
                broker.handleError(new errors_1.WeaveError('Message payload missing!'));
            }
            ...
            switch (type) {
                case message_types_1.default.MESSAGE_DISCOVERY:
                    onDiscovery(payload);
                    break;
                case message_types_1.default.MESSAGE_INFO:
                    onNodeInfos(payload);
                    break;
                case message_types_1.default.MESSAGE_REQUEST:
                    onRequest(payload);
                    break;
                case message_types_1.default.MESSAGE_RESPONSE:
                    onResponse(payload);
                    break;
                case message_types_1.default.MESSAGE_PING:
                    onPing(payload);
                    break;
                case message_types_1.default.MESSAGE_PONG:
                    onPong(payload);
                    break;
                case message_types_1.default.MESSAGE_DISCONNECT:
                    onDisconnect(payload);
                    break;
                case message_types_1.default.MESSAGE_HEARTBEAT:
                    onHeartbeat(payload);
                    break;
                case message_types_1.default.MESSAGE_EVENT:
                    onEvent(payload);
                    break;
            }
            return true;
