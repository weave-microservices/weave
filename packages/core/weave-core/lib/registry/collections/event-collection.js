...
function createEventCollection(registry) {
    ...
    eventCollection.add = (node, service, event) => {
        ...
        if (!endpointList) {
            endpointList = endpoint_collection_1.createEndpointCollection(broker, event.name, groupName);
    ...
    eventCollection.list = ({ onlyLocals = false, skipInternals = false, withEndpoints = false } = {}) => {
        ...
        events.forEach(list => {
            ...
            if (item.count > 0) {
                ...
                if (endpoint) {
                    item.event = utils_1.omit(endpoint.action, ['handler', 'service']);
                }
            }
            if (withEndpoints) {
...
exports.createEventCollection = createEventCollection;
;
