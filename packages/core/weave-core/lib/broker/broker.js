...
function createBroker(options) {
    ...
    const createLogger = (moduleName, service) => {
        ...
        if (process.env.NODE_ENV === 'production') {
            options.logger.logLevel = options.logger.logLevel || log_level_enum_1.LogLevel.Info;
        }
        else if (process.env.NODE_ENV === 'test') {
            options.logger.logLevel = options.logger.logLevel || log_level_enum_1.LogLevel.Error;
        }
        else {
            options.logger.logLevel = options.logger.logLevel || log_level_enum_1.LogLevel.Debug;
        }
