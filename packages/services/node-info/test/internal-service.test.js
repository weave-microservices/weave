...
describe("Test internal service $node", () => {
  it('Five actions from "$node" should be available.', (done) => {
    ...
    broker1.start().then(() => {
      broker1.call("$node.actions", { withActions: true }).then((res) => {
  ...
  it("shlould get one service from service node.", (done) => {
    ...
    broker1.start().then(() => {
      broker1.call("$node.services", { withNodeService: true }).then((res) => {
  ...
  it("shlould get no event node service.", (done) => {
    ...
    broker1.start().then(() => {
      broker1.call("$node.events", { withNodeService: true }).then((res) => {
  ...
  it("shlould get a list of all connected nodes", (done) => {
    ...
    broker1.start().then(() => {
      broker1.call("$node.list").then((res) => {
        expect(res.length).toBe(1);
        done();
      });
    });
